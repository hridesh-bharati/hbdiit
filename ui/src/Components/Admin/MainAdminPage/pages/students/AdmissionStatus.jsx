  ...
  const handleAction = async () => {
    if (modalType === 'delete') {
      ...
      try {
        ...
        toast.error("Failed to take admission.");
      }
              ...
              {allStudent.map(student => (
                <tr key={student._id}>
                  ...
                  <td>
                    ...
                    {!student.regNum && <button className="btn btn-sm text-white " title='Delete Registration' onClick={() => { setModalType('delete'); setStudentId(student._id); setShowModal(true); }}><i className="bi bi-trash-fill text-danger"></i></button>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <Modal show={showModal} onHide={closeModal} animation={false}>
          <Modal.Header className='bg-primary' closeButton>
            <Modal.Title className="text-white text-uppercase">
              {modalType === 'delete' ? "Delete Student's" : modalType === 'admission' ? 'Take Admission' : "Generate Certificate"}
            </Modal.Title>
          </Modal.Header>
          <Modal.Footer>
